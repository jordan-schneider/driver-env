...
def _f(x, shape=5.0):
    ...
    return tf.where(x > 0, tf.exp(-1 / (shape * x_clipped)), tf.zeros_like(x))
...
def gaussian(mean, stddev):
    ...
    def g(x):
        return (
            1
            / ((2 * np.pi) ** 0.5 * stddev)
            * tf.exp(-tf.reduce_sum((x - mean) ** 2, axis=-1) / (2 * stddev ** 2))
        )

    return g


def smooth_threshold(threshold, width, c=5.0):
    """
    ...
    def t(x):
        ...
        return _f(x_diff, shape) / (_f(x_diff, shape) + _f(width - x_diff, shape))
...
def smooth_bump(start, end):
    ...
    def bmp(x):
        ...
        return tf.where(cond, tf.exp(-1 / (1 - x_norm_clipped ** 2) + 1), 0)

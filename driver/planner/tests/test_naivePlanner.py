...
class TargetSpeedTestCases(unittest.TestCase):
    def setUp(self):
        ...
        self.init_state = np.array([0.0, 0.0, 1.0, np.pi / 2], dtype=np.float32)

    def test_zero_friction_correct_speed(self):
        car = TargetSpeedPlannerCar(self.world, self.init_state, 4, target_speed=1.0, friction=0.0)
        self.world.add_car(car)

        planner = NaivePlanner(self.world, car, horizon=5, learning_rate=5.0, n_iter=100)
    ...
    def test_friction_correct_speed(self):
        friction = 0.5
        car = TargetSpeedPlannerCar(
            self.world, self.init_state, 4, target_speed=1.0, friction=friction
        )
        self.world.add_car(car)

        planner = NaivePlanner(self.world, car, horizon=3, learning_rate=5.0, n_iter=500)
        plan = planner.generate_plan([car.state])
        self.assertEqual(len(plan), 3)
        for i in range(len(plan)):
            np.testing.assert_allclose(
                plan[i].numpy(), np.array([friction * 1.0 ** 2, 0.0]), atol=1e-5
            )
...
class TestPlanVsFixedPlanCar(unittest.TestCase):
    ...
    def setUp(self):
        self.world = ThreeLaneCarWorld()
        self.init_state = np.array([0.0, 0.0, 1.0, np.pi / 2], dtype=np.float32)
        self.other_car_init_state = np.array([0.1, 0.0, 1.0, np.pi / 2], dtype=np.float32)

    def test_no_interaction(self):
        car = TargetSpeedPlannerCar(self.world, self.init_state, 4, target_speed=1.0, friction=0.0)
        other_car = FixedPlanCar(
            self.world,
            self.other_car_init_state,
            plan=[np.array([0.0, 0.0], dtype=np.float32)],
        ...
        plan = planner.generate_plan(
            [car.state, other_car.state], other_controls=[tf.constant(0.0), other_plan]
        )

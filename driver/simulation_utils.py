...
def car_dynamics_step(x, y, v, angle, acc, ang_vel, dt, friction):
    acc = tf.maximum(tf.minimum(acc, 4.0), -2 * 4.0)
    ang_vel = tf.maximum(tf.minimum(ang_vel, 4), -4.0)
...
def batched_next_car_state(
    ...
    if len(control.shape) != 2 or control.shape[1] != 2:
        raise ValueError("Input control has incorrect shape {}".format(control.shape))

    x, y, v, angle = state[:, 0], state[:, 1], state[:, 2], state[:, 3]
    acc, ang_vel = control[:, 0], control[:, 1]

    new_x, new_y, new_v, new_angle = car_dynamics_step(x, y, v, angle, acc, ang_vel, dt, friction)
...
def next_car_state(
    ...
    if legacy:
        ...
        angle_vel, acc = control[0], control[1]

        new_x, new_y, new_v, new_angle = legacy_car_dynamics_step(
            x=x, y=y, v=v, angle=angle, acc=acc, angle_vel=angle_vel
        )

        return tf.stack([new_x, new_y, new_angle, new_v], axis=-1)
    else:
        x, y, v, angle = state[0], state[1], state[2], state[3]
...
class CarState(object):
    ...
    def _check_state_is_valid(self):
        if self.state.shape[0] != 4:
            raise ValueError("Input state has incorrect shape {}".format(self.state.shape))
...
class BatchedCarState(CarState):
    ...
    def _check_state_is_valid(self):
        if len(self.state.shape) != 2 or self.state.shape[1] != 4:
            raise ValueError("Input state has inccorrect shape {}".format(self.state.shape))
...
class CarControl(object):
    def __init__(self, numpy_or_tf_controls):
        ...
        """
        self.control = tf.Variable(numpy_or_tf_controls, name="control", dtype=tf.float32)
        self._check_control_is_valid()

    def _check_control_is_valid(self):
        if self.control.shape[0] != 2:
            raise ValueError("Input control has incorrect shape {}".format(self.control.shape))
...
class BatchedCarControl(CarControl):
    ...
    def _check_control_is_valid(self):
        if len(self.control.shape) != 2 or self.control.shape[1] != 2:
            raise ValueError("Input state has incorrect shape {}".format(self.control.shape))
...
def new_next_car_state(
    state: CarState, control: CarControl, dt: float, friction: float = 0.0, inplace: bool = False
) -> CarState:
    """
    ...
    new_state_value = tf.stack(
        car_dynamics_step(
            state.x, state.y, state.v, state.angle, control.acc, control.ang_vel, dt, friction
        ),
        axis=-1,
    )
...
def get_dynamics_fn(friction: float, legacy: bool):
    @tf.function

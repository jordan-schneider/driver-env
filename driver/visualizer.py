...
from driver.car import Car, LinearRewardCar
...
class CarVisualizer(object):
    ...
    def __init__(
        ...
        if window_args is None:
            window_args = dict(caption=name, height=600, width=600, fullscreen=False)
        else:
    ...
    def _draw_world(self):
        ...
        for car in self.world.cars[
            ::-1
        ]:  # main robot car is generally first; draw them backwards to draw it on top
            # if car.debug:
            ...
            for t, (state_tf, control_tf) in enumerate(car.past_traj):
                ...
                if use_subframes:
                    if t < len(car.past_traj) - 1:
                        ...
                        sprite.opacity = (
                            car.opacity * opacity ** (len(car.past_traj) - t - 0.5) * 255
                        )
                        sprite.draw()
                    else:
